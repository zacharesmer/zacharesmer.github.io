...
Each page in the nav bar is a markdown or html file in the root directory. The nav bar is generated by iterating through entries in `_data/nav.yml`. To make a new page, copy `_templates/main-page` into the root directory (it will work with html or markdown). Then add an entry to `_data/nav.yml` in the form:
    `\- name: `
    `  link: `
